...
export default async function handler(req, res) {
  ...
  switch (method) {
    ...
    case "POST":
      try {
        let newRecord;
        const allRecords = await Referral.find().sort({ createdAt: -1 });
        if (allRecords && allRecords.length > 0) {
          newRecord = await Referral.create({
            ...req.body,
        ...
        let messageData = {
          ...
          <br />
          <b>YOUR UNIQUE REFERRAL REFERENCE NUMBER: UD/${
            newRecord.reference
          }/${moment(new Date()).format("YYYY")} </b> <br /><br />

          Thank you for submitting the candidate referral form. One of our recruitment professionals will contact you within the next 48 working hours. <br />
